 ...
 */
public class Tester {
    public static final String PROBLEM = "mincross";
    ...
    public static boolean filesEqual(File solutionOutputFile, File testOutputFile) throws IOException {
        try (
            Scanner solutionOutput = new Scanner(new BufferedInputStream(new FileInputStream(solutionOutputFile)));
    ...
    public static void extractFilesHelper(File unzipDir) throws IOException {
        ...
        while (zEntries.hasMoreElements()) {
            ...
            Files.copy(zip.getInputStream(curr),
                    new File(unzipDir.getCanonicalPath() + "/" + curr.getName()).toPath());
        }
    ...
    public static File getZipFile() {
        return Arrays.stream(DATA_DIR.listFiles())
                .filter(f -> f.getName().endsWith(".zip"))
                .filter(f -> f.getName().contains(PROBLEM))
                .findAny()
                .orElseThrow(() -> new IllegalArgumentException(
                        String.format("Missing data for %s.", PROBLEM)));
    }
}
